...
- Maps TodoWrite-style todo updates to ACP `plan` updates (full replacement semantics)
  - Emits empty `entries: []` when todos are cleared
  - On `session/load`, replays only the latest todo plan snapshot
  - Plan entry IDs are emitted only when client capabilities indicate support (`_meta.plan-entry-ids` / `_meta.plan_entry_ids` / `_meta.plan-entry-id`)
